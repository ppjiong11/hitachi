#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""Standalone door open/close test for Hitachi CN."""

from __future__ import annotations

from hitachi_cn.app import run_field_runner
from hitachi_cn.config import clone_config


CONFIG = clone_config(
    BASE_URL="http://192.168.88.254:8081",
    USERNAME="WVCOND",
    PASSWORD="WV12345",
    VERIFY_TLS=False,
    LIFT_ID=1,
    USE_REAR=False,
    RUN_SNAPSHOT=False,
    RUN_DOOR_TEST=True,
    RUN_TRIP_TEST=False,
    RUN_RESET_ONLY=False,
    # True = only print the command, False = really send it to the lift.
    DRY_RUN=False,
    CONFIRM_BEFORE_MOVE=True,
    ALLOW_NON_NORMAL_MODE=False,
    NORMAL_OPERATION_MODE=4,
    POLL_INTERVAL_SEC=0.5,
    STABLE_N=2,
    REGISTER_TIMEOUT_SEC=10.0,
    # Door test mode:
    # "1" or "open"  -> only send open command
    # "0" or "close" -> only send close command
    # "cycle"        -> open, hold, then close
    DOOR_TEST_ACTION="open",
    DOOR_HOLD_SEC=10.0,
)


def main() -> None:
    run_field_runner(CONFIG)


if __name__ == "__main__":
    main()



#二号是 fire mode